...
> Type: `inprogress`
> Updated: `2026-04-28`
> Summary: 合并此前三份 Claude 设计文档后，继续按当前仓库代码与最新 feidex 复核，收敛 Claude backend 基座缺口、命令策略边界与关键交互护栏，并补充 2026-04-28 的两种推进方案与推荐顺序。
...
2. 剩余 pre-MVP 小包按 `#493 -> #494 -> #495` 顺序推进：
   - `#493`：`#369` 的 registry / binding / context repair
   - `#494`：command strategy / request bridge / final-output contract
   - `#495`：runtime host / Claude dark-launch skeleton / reconciliation
3. 三张 pre-MVP 子单收口后，明确停在 `#496` 这个 MVP 决策门，先讨论 visible 入口、第一批命令和飞书展现方式，再开做最小可见 dev MVP。
4. 等 dev MVP 跑通，再决定哪些命令从 hidden / reject 升级成一等本地能力。
