...
8. command strategy matrix（native / approximation / passthrough / reject）仍未形成 durable contract。
...
2. command strategy matrix（native / approximation / passthrough / reject）仍未收口。
...
5. `codex <-> claude` 切换时会走 detach-like 清理，并显式清掉旧 backend 的 resume target；但 normal-mode 的 managed-headless exact-thread continuation 已不再是 Codex-only，Claude 也走同一条恢复主链，只把 backend 差异留在 runtime-native resume mechanics。
6. normal-mode attach / auto-resume / visible-instance resolver 已开始按 backend 隔离，避免把 `codex` 的 thread/workspace 恢复到 `claude`，反之亦然。
...
`#492` 已经收口了 `/mode` 的底层状态语义与 normal-mode backend 隔离：
...
2. Claude 真正可见后，config 入口还需要跟 command strategy matrix 一起决定哪些 visible、哪些 hidden/reject。
...
| `assistant.tool_use`（外部工具） | 工具调用开始，已拿到稳定 `tool_use_id + name + input` | `item.started` with `itemKind=dynamic_tool_call` + metadata(`tool`,`arguments`,`toolUseID`) | `dynamic_tool_call` 足够承接第一版，不必先拆成更多专属 item kind |
| `user.tool_result`（外部工具） | 工具执行完成，携带 stdout/stderr/error/image/interrupt 等真实结果 | `item.completed` with `itemKind=dynamic_tool_call` + structured metadata | 这是工具完成的主 carrier，不能再把 assistant `tool_use` 误当 completed |
...
3. `#497` 第一版应优先复用 `dynamic_tool_call`。
   - Claude 非内部工具先统一投影成 `dynamic_tool_call started/completed`。
   - 是否把 Bash/Web/Search/Write 再细分回 `command_execution` / `web_search` / `file_change`，留作后续增强，不作为 pre-MVP 阻塞项。
