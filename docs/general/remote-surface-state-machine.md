...
3. `G2 PendingRequest` 的 runtime source of truth 现在已经从 `Phase` / `PendingDispatchCommandID` 散写，收口到 `RequestPromptRecord.LifecycleState`：
   ...
   3. `FeishuRequestView.Phase` 现在只是 frontstage 投影：`submitting` 与 `awaiting_backend_consume` 都映射成只读 `waiting_dispatch`；`PendingDispatchCommandID` 只保留为“本地尚未收到 accept/reject 的关联键”，不再是 gate source of truth。
   4. 因此 `command_ack.accepted` 后，即使 `PendingDispatchCommandID` 已清空，surface 仍会继续被 `awaiting_backend_consume` gate 挡住，直到上游 `request.resolved` 或 owner terminal 路径显式 abort。
   5. turn complete、detach、route lost、instance offline/transport degraded 这类 owner terminal 清理路径，当前都会先把命中的 request 标成 `aborted(+expired/cancelled phase)`，再移出 `PendingRequests`，不再直接 silent delete。
