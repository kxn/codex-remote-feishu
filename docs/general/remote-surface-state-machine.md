...
5. daemon 在 fresh/preselected headless launch 时会把 `CODEX_REMOTE_INSTANCE_BACKEND=<surface backend>` 注入环境，因此 Claude headless 会按 Claude backend 启动，而不是默回 Codex。
