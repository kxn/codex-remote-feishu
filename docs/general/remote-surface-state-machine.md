...
5. `常用工具` 分组里的 `/cron`，以及 `系统管理` 分组里的 `/debug`、`/upgrade`、`/restart`、`/bendtomywill` 当前仍然是直接触发 daemon 动作的命令入口，不属于参数卡表单。
6. `/restart` 当前明确拆成 page / execute 两类：
...
| `/help` `/menu` `/debug` `/upgrade` `/restart` | 允许 | 允许 | 允许 | 允许 | 允许 | 允许 |
