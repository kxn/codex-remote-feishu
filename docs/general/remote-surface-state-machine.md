...
> Type: `general`
> Updated: `2026-08-11`
> Summary: 同步 workspace/path identity SSOOT：PendingHeadless 的 workspaceKey/threadCWD 组合统一由 resolver 判定，路径身份比较统一走 canonical path compare；并保留 route core、旧 `attach_instance` fail-closed、target picker、OpenCode backend、Profile-first Codex、room workspace、headless recovery、动态模型菜单、prompt override、跨模型组 route restart、typed Codex resume policy 等既有状态机合同。
