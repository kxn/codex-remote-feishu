...
| `M1 ClaudeNormal` | `ProductMode=normal`，`Backend=claude` | 仍走 workspace-first normal-mode 语义，但 workspace defaults、surface resume 与 detached catalog context 都按 Claude backend 分区；当前不进入 Codex 的 VS Code / headless 恢复语义 |
...
2. `ProductMode` / `Backend` 当前已经进入 daemon 级 `surface resume state`：
   ...
   3. `surface resume state` 当前不仅记录 `ProductMode` / `Backend` / `Verbosity` / `PlanMode` / instance / thread / workspace / route，还会记录 headless 恢复所需的 thread title / thread cwd / headless 标记；它已经是唯一持久化恢复源。旧 entry 缺失 `Backend` 时会 lazy 默认成 `codex`。
...
3. `/mode` 当前只在没有 live remote work 的 surface 上执行切换：
   ...
   3. 如果切换前后发生了 backend 或 `ProductMode` 变化，会清掉 `surface resume state` 里的旧 resume target，避免 `codex` / `claude` 之间串恢复。
   4. 如果当时还带着 `PendingHeadless`，会先显式 kill 当前 headless 启动流程，并清掉 `surface resume state` 里的 headless 恢复目标与内存恢复状态。
...
1. 允许：`/status`、`/autowhip`、`/autocontinue`、`/debug`、`/upgrade`、`/repair`、`/mode`、`/detach`、消息撤回、reaction。
...
3. 切换时一定先做 detach-like 清理，再进入目标 mode/backend 的 detached 态；workspace claim 也会一起释放。
...
5. `常用工具` 分组里的 `/cron`，以及 `系统管理` 分组里的 `/debug`、`/upgrade`、`/repair`、`/restart`、`/bendtomywill` 当前仍然是直接触发 daemon 动作的命令入口，不属于参数卡表单。
6. `/repair` 是断联后的当前 surface 一键恢复命令：
   1. 它不打开根页，纯文本或菜单/card 入口都会立即 append `Repair` 开始提示，并在后台执行。
   2. 它总是先按当前 gateway 读取既有 runtime 配置并调用飞书 bot runtime apply；缺少 gateway 或配置时只跳过 bot runtime 重连，不把这一步扩成全局配置修复。
   3. 若当前 attached instance 在线、idle、没有 abandoning/queued/running turn、且没有同 instance child restart 在飞，则复用 `/restart child` 的 `process.child.restart` 两段式合同重启当前 provider child。
   4. 若当前 attached instance 是离线 managed headless，且 surface/instance 上仍有 `threadID + CWD` 恢复锚点，则移除旧离线 instance 并触发现有 headless auto-restore。
   5. 若 provider 侧 busy、离线但不可恢复、或状态已变化，则只跳过 provider child/headless 步骤并报告原因；飞书 bot runtime 重连仍会先执行。
   6. 默认 `/repair` 不重启 daemon，不 kill unrelated/private/interactive 实例，也不修改 `~/.codex/auth.json` 或 `~/.codex/config.toml`。
   7. `/repair daemon` 是显式 daemon restart fallback：它不执行 bot runtime 重连或 provider child/headless 修复，只读取当前 install-state；若当前 daemon 由 `systemd_user` lifecycle manager 托管，则通过 install lifecycle 发起 restart，否则拒绝并提示需要 server/supervisor 操作。
7. `/restart` 当前明确拆成 page / execute 两类：
   1. bare `/restart` 打开重启根页，只暴露一个 `/restart child` 按钮。
   2. `/restart child` 只重启当前 attached instance 的 provider child，不重启 daemon。
   3. 若当前未接管实例、实例离线、surface 仍有 queued/running turn、正在 abandoning，或同一 instance 已有 child restart 在飞，则根页按钮会禁用，直接执行也只回明确拒绝提示。
...
| `/help` `/menu` `/debug` `/upgrade` `/repair` `/restart` | 允许 | 允许 | 允许 | 允许 | 允许 | 允许 |
...
| `/repair` | 允许；异步重连当前飞书 bot runtime；没有 attached instance 时跳过 provider 侧修复 | 允许；异步重连当前飞书 bot runtime；provider 侧只在 idle/安全时重启当前 attached child，离线 managed headless 有恢复锚点时走 auto-restore，否则跳过并提示 | 允许；异步重连当前飞书 bot runtime；provider 侧只在 idle/安全时重启当前 attached child，离线 managed headless 有恢复锚点时走 auto-restore，否则跳过并提示 | 允许；异步重连当前飞书 bot runtime；provider 侧只在 idle/安全时重启当前 attached child，离线 managed headless 有恢复锚点时走 auto-restore，否则跳过并提示 | 允许；异步重连当前飞书 bot runtime；provider 侧只在 idle/安全时重启当前 attached child，离线 managed headless 有恢复锚点时走 auto-restore，否则跳过并提示 | 允许；异步重连当前飞书 bot runtime；provider 侧只在 idle/安全时重启当前 attached child，离线 managed headless 有恢复锚点时走 auto-restore，否则跳过并提示 |
| bare `/restart` | 允许，返回重启根页；未接管时 `/restart child` 按钮禁用，并提示先 `/list` 或 `/workspace` 重新接入 | 允许，返回重启根页；若实例离线、surface busy、abandoning，或已有 child restart 在飞，则按钮禁用并显示原因 | 允许，返回重启根页；若实例离线、surface busy、abandoning，或已有 child restart 在飞，则按钮禁用并显示原因 | 允许，返回重启根页；若实例离线、surface busy、abandoning，或已有 child restart 在飞，则按钮禁用并显示原因 | 允许，返回重启根页；若实例离线、surface busy、abandoning，或已有 child restart 在飞，则按钮禁用并显示原因 | 允许，返回重启根页；若实例离线、surface busy、abandoning，或已有 child restart 在飞，则按钮禁用并显示原因 |
| `/restart child` | 拒绝并提示先 `/list` 或 `/workspace` 重新接入 | 允许；若当前 attached instance 离线、surface busy、abandoning，或已有 child restart 在飞，则拒绝并提示；否则异步重启当前 attached instance 的 provider child，不重启 daemon | 允许；若当前 attached instance 离线、surface busy、abandoning，或已有 child restart 在飞，则拒绝并提示；否则异步重启当前 attached instance 的 provider child，不重启 daemon | 允许；若当前 attached instance 离线、surface busy、abandoning，或已有 child restart 在飞，则拒绝并提示；否则异步重启当前 attached instance 的 provider child，不重启 daemon | 允许；若当前 attached instance 离线、surface busy、abandoning，或已有 child restart 在飞，则拒绝并提示；否则异步重启当前 attached instance 的 provider child，不重启 daemon | 允许；若当前 attached instance 离线、surface busy、abandoning，或已有 child restart 在飞，则拒绝并提示；否则异步重启当前 attached instance 的 provider child，不重启 daemon |
...
| 覆盖状态 | 当前行为 |
| --- | --- |
| `G1 PendingHeadlessStarting` | 只允许 `/status`、`/autowhip`、`/autocontinue`、`/debug`、`/upgrade`、`/repair`、`/mode`、`/detach`、revoke/reaction；其中 `/repair` 会继续派发到 daemon 做当前 surface 修复，不会被 pending headless notice 吃掉；`/mode` 若实际切到了新的 backend 或 `ProductMode`，会直接 kill 当前恢复流程并清空持久化 headless / resume target；reaction 即使放行到 action 层，也只会在满足 steering 条件时生效。若当前 pending 只是后台 auto-restore 占位，手动 `/upgrade latest` 与允许 dev feed 的 flavor（源码 `dev` 与 release `alpha`）下的 `/upgrade dev` 允许继续弹候选升级卡 |
...
| `/repair` | `ActionRepairCommand -> DaemonCommandRepair` | 当前 surface 的一键恢复命令；默认后台重连当前飞书 bot runtime，安全时复用当前 attached provider child restart 或 managed headless auto-restore；`/repair daemon` 只在当前 daemon 由 install lifecycle manager 托管时发起 daemon restart；不修改全局 Codex auth/config |
...
3. `/help` 这类静态目录卡、apply 终态、request prompt 终态，以及 bare `/upgrade` / `/debug` 的状态卡与 upgrade 重启后结果 notice 等仍然沿用 append-only 消息语义，不在这轮同步回包范围内；`/restart` 当前也不在这条 inline-view allow-list 里，但 bare `/restart` 若来自 stamped 当前卡，会走 `first_result_card` 把重启根页直接替回当前卡，而 `/restart child` 继续保持 append-only。`/upgrade latest` 一旦进入 daemon owner-card 流，也会在同一张升级卡上继续 patch 到 confirm/running/restarting。
