...
6. `/claudeprofile` 当前是 Claude headless 专属的 surface 级切换入口：
   ...
   5. 当前 workspace 已占用时，切 profile 会先 detach-like 清理旧 runtime，再按目标 `workspace+profile` 快照恢复 `PlanMode / ReasoningEffort / AccessMode`，随后按切换前的 continuation intent 直接 fresh-restart 到新 profile：原来只是 workspace-owned `unbound/new_thread_ready` 时继续走 fresh workspace prepare；原来已经 pinned 到某个 Claude thread 时，会保留该 exact-thread 恢复目标并直接拉起新的 thread-restore headless，而不会要求用户重新 `/use`。
...
8. daemon 在 fresh/preselected headless launch 时会把 `CODEX_REMOTE_INSTANCE_BACKEND=<surface backend>` 注入环境；若当前 backend 是 `claude`，还会把当前 `ClaudeProfileID` 一并下发到 headless command，因此 Claude headless 会按正确 profile 启动，而不是默回 Codex 或默回别的 Claude profile。
