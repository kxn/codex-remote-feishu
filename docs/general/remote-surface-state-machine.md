...
6. 当前工作会话命令已经按 mode 分流：
   ...
   2. `claude normal` 的 visible MVP 继续复用同一套命令目录壳，但当前只显式开放 `/new`、`/list`、`/use`；`workspace*` 与 `/useall` 继续 hidden + reject。
...
### 4.1.1 claude normal visible MVP 继续复用同一工作会话壳，但只开放 `/new` `/list` `/use`
...
1. `ResolveFeishuCommandStrategy()` 与 `ResolveFeishuCommandDisplayProfileForContext()` 会把 Claude visible MVP 固定为：
   ...
   3. `workspace*` 与 `/useall` 继续 hidden + reject，不会在 Claude mode 下伪装成可用入口。
...
2. 二级分组顺序稳定，但组内可见命令会按当前 `product mode + menu stage` 做 display projection：
   ...
   8. `/new` 与 `/bendtomywill` 只在 `normal` working 可见；`/status` 当前在 `基本命令`，`/history` 在 `常用工具`，`/bendtomywill` 在 `系统管理`，其中 `/status` 与 `/history` 在 normal / vscode 都可见。
3. `/help` 当前也复用同一套 display projection：
   ...
   2. `claude normal` 下帮助文本的主展示入口当前只保留 `/list`、`/use` 与 `/new` 这组 visible MVP 会话主链；`workspace*` 与 `/useall` 仍只存在于 parser / reject 兼容层，不作为主展示帮助入口。
