...
| `M1 ClaudeNormal` | `ProductMode=normal`，`Backend=claude` | 仍走 workspace-first normal-mode 语义，但 workspace defaults、surface resume 与 detached catalog context 都按 Claude backend 分区；surface 还会额外携带当前 `ClaudeProfileID`，并按 `workspace+profile` 恢复 `reasoning / plan / access`；当前不进入 Codex 的 VS Code / headless 恢复语义 |
...
3. `/mode` 当前只在没有 live remote work 的 surface 上执行切换：
   ...
   3. 如果目标是 `claude normal` 且切换前已经存在当前 workspace，则 surface 会恢复这份 workspace claim，并立即优先 attach 同 workspace 的在线 Claude instance；若当前还没有可 attach 的 Claude instance，则直接启动 fresh managed headless，不再停在 detached idle。这个 Claude fresh headless 会显式走 `claude-app-server`，而不是复用 Codex `app-server` 入口再靠 env 猜 backend。
...
5. `/claudeprofile` 当前是 Claude normal 专属的 surface 级切换入口：
