...
7. instance claim 与 thread claim 仍是 surface 级全局独占，同 room 不共享实例或会话；当前也不改变 queue、route mode 或 Feishu 可见交互。
