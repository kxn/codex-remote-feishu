...
8. 当 `ReviewSession.ActiveTurnID` 非空时，这层 overlay 还会进入统一 route-mutation blocker seam：`/use`、`/follow`、`/new`、`/claudeprofile`、`/codexprofile`、`/compact` 等会改工作目标的动作都会直接拒绝，并返回 `review_running`；只有 idle review session 会在 detach-like cleanup 或 route change 时被自动清掉。
...
8. daemon 在 fresh/preselected headless launch 时，会只消费 orchestrator 已冻结下来的 headless launch contract：
   1. `PendingHeadless` 与 `DaemonCommandStartHeadless` 当前都会显式携带 `Backend / CodexProfileID / ClaudeProfileID / OpenCodeProfileID / OpenCodeAdmissionRef`；若 frozen backend 是 `claude`，还会额外带 `ClaudeReasoningEffort`。
