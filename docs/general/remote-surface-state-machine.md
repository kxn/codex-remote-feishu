...
| `G2 PendingRequest` | `PendingRequests` 非空 | 普通文本/图片/文件会被待处理请求门禁挡住；当前可能是 approval、`approval_command`、`approval_file_change`、`approval_network`、`request_user_input`、`permissions_request_approval`、`mcp_server_elicitation` 或 `tool_callback`。顶层 `tool/requestUserInput` 与 `item` 形式共用同一 `request_user_input` gate；`tool_callback` 当前不会等待用户作答，而是以只读提示 + 自动 unsupported 回写的 fail-closed 方式短暂占用 gate，直到上游 `request.resolved` 清理 |
