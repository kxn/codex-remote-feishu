...
| `M1 HeadlessClaude` | `ProductMode=normal`，`Backend=claude` | headless 主链的 Claude 分支。workspace defaults、surface resume 与 detached catalog context 都按 Claude backend 分区；surface 还会额外携带当前 `ClaudeProfileID`，并按 `workspace+profile` 恢复 `reasoning / plan / access`；不进入 VS Code 语义，但已经共享 headless exact-thread 恢复主链，并在需要时通过 Claude 原生 `--resume` 恢复旧 session |
...
5. 选择工作区、选择会话，或从主卡打开 path picker 子步骤时，只会刷新 target picker 本身或其子步骤，不会立即 attach / switch。
   ...
   3. `target_picker_select_mode` / `target_picker_select_source` 仍保留 transport 兼容，但当前 headless 主路径默认不会命中。
...
8. Claude profile 只改变启动时注入给 wrapper/Claude child 的 endpoint、auth token 与 model env；它不会改写或创建 profile 专属 `CLAUDE_CONFIG_DIR`。因此 exact-thread 恢复、session catalog 与 history 继续共享同一个 Claude 会话目录视图，不能因为切换 `ClaudeProfileID` 把同一组 session 分裂成多套。
...
| `/workspace` `/workspace new` | `codex headless`: 允许，分别打开工作会话父页 / 新建方式页；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开工作会话父页 / 新建方式页；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开工作会话父页 / 新建方式页；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开工作会话父页 / 新建方式页；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开工作会话父页 / 新建方式页；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开工作会话父页 / 新建方式页；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless |
| `/workspace list` `/workspace new dir` `/workspace new git` `/workspace new worktree` | `codex headless`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless | `codex headless`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`claude headless`: hidden + reject；`vscode`: 拒绝并提示先切到 headless |
...
| `target_picker_select_mode` | `ActionTargetPickerSelectMode` | 兼容保留的模式页回调；当前 headless 主路径默认不会命中 |
| `target_picker_select_source` | `ActionTargetPickerSelectSource` | 兼容保留的来源页回调；当前 headless 主路径默认不会命中 |
