...
1. `ResolveFeishuCommandStrategy()` 与 `ResolveFeishuCommandDisplayProfileForContext()` 会把 Claude visible MVP 固定为：
   ...
   3. `common_tools` 当前显式开放 `/review`、`/bendtomywill` 与 `/history`。
   4. `workspace*` 与 `/useall` 继续 hidden + reject，不会在 Claude mode 下伪装成可用入口。
...
5. `ClaudeProfileID` 当前也是 surface runtime carrier：
...
6. daemon 在 fresh/preselected headless launch 时会把 `CODEX_REMOTE_INSTANCE_BACKEND=<surface backend>` 注入环境；若当前 backend 是 `claude`，还会把当前 `ClaudeProfileID` 一并下发到 headless command，因此 Claude headless 会按正确 profile 启动，而不是默回 Codex 或默回别的 Claude profile。
