...
### 4.1.1 当前 live Claude dev MVP 继续复用同一工作会话壳，并额外开放 review / patch 工具入口
...
1. `ResolveFeishuCommandStrategy()` 与 `ResolveFeishuCommandDisplayProfileForContext()` 会把 Claude visible MVP 固定为：
   ...
   4. `common_tools` 当前显式开放 `/review`、`/bendtomywill` 与 `/history`。
   5. `workspace*`、`/useall`、`/plan` 与 `/autocontinue` 继续 hidden + reject，不会在 Claude mode 下伪装成可用入口。
2. `/list` 与 `/use` 当前继续复用 normal-mode target picker / workspace-session shell，不另造一套 Claude 专用卡。
3. 但 target picker 的 workspace/session 候选已经按当前 backend 过滤：
   ...
   3. Claude normal 不再把 persisted Codex recent threads/workspaces 混进 `/list` / `/use` 候选。
...
2. 二级分组顺序稳定，但组内可见命令会按当前 `product mode + menu stage` 做 display projection：
   ...
   5. `claude normal` 下 `switch_target` 分组当前直接显示 `/list`、`/use`；`workspace*` 与 `/useall` 不再出现在主展示菜单里。
   6. `vscode mode` 的 `工作会话` 仍分别显示 `/list`、`/use`、`/useall`。
   7. `normal mode` 不展示 `/follow`；`vscode mode` 才展示 `/follow`。
   8. `/new` 与 `/bendtomywill` 只在 `normal` working 可见；`/status` 当前在 `基本命令`，`/history` / `/review` / `/bendtomywill` 在 `常用工具`，其中 `/status` 与 `/history` 在 normal / vscode 都可见。
3. `/help` 当前也复用同一套 display projection：
   ...
   2. `claude normal` 下帮助文本的主展示入口当前会保留 `/new`、`/list`、`/use` 这组 visible MVP 会话主链，并在 `常用工具` 展示 `/review`、`/bendtomywill` 与 `/history`；`workspace*` 与 `/useall` 仍只存在于 parser / reject 兼容层，不作为主展示帮助入口。
