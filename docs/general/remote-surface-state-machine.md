...
3. `/mode` 当前只在没有 live remote work 的 surface 上执行切换：
   ...
   3. 如果目标是 `claude normal` 且切换前已经存在当前 workspace，则 surface 会恢复这份 workspace claim，并立即优先 attach 同 workspace 的在线 Claude instance；若当前还没有可 attach 的 Claude instance，则直接启动 fresh managed headless，不再停在 detached idle。
