...
6. `/repair` 是断联后的当前 surface 一键恢复命令：
   ...
   6. 它不重启 daemon，不 kill unrelated/private/interactive 实例，也不修改 `~/.codex/auth.json` 或 `~/.codex/config.toml`。
...
| `/repair` | `ActionRepairCommand -> DaemonCommandRepair` | 当前 surface 的一键恢复命令；后台重连当前飞书 bot runtime，安全时复用当前 attached provider child restart 或 managed headless auto-restore；不重启 daemon，不修改全局 Codex auth/config |
