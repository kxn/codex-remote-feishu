...
2. 群聊消息必须在 `mentions` 中命中当前 gateway 缓存的 bot `open_id`，才允许 materialize / reuse `feishu:<gatewayID>:chat:<chatID>` surface。
3. 群聊无 mention、只 mention 其他 bot、或当前 gateway 尚未拿到自己的 bot `open_id` 时，消息会 fail closed 忽略，不记录 `messageID -> surfaceID`，不进入 queue / dispatch。
4. 当前 bot `open_id` 在 gateway 启动时通过 bot info API 获取并缓存，普通消息热路径只读取缓存，不逐条调用飞书 API。

Feishu 群聊 surface 之上现在还有一层 room context coordination record：

1. 群聊 surface materialize/resume 时会按 `chatID` 维护 `FeishuRoomContexts[feishu:chat:<chatID>]`。
2. record 当前只保存 room id、`chatID`、参与过的 gateway id evidence 与 surface session id evidence。
...
5. 这层 record 当前不改变 workspace claim、instance/thread claim、queue、route mode 或 Feishu 可见交互；它只为后续 room workspace binding、room reset 与 room active lock 提供枚举同 room surfaces 的 SSOT。
...
5. `常用工具` 分组里的 `/cron`、`/bendtomywill`，以及 `系统管理` 分组里的 `/debug`、`/upgrade` 当前仍然是直接触发 daemon 动作的命令入口，不属于参数卡表单；`/mcpoauth` 也是直接 daemon command，但当前不进菜单，只在 slash/help 路径暴露。
