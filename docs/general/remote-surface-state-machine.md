...
> Type: `general`
> Updated: `2026-04-30`
> Summary: 当前实现同步了 workspace-aware normal mode 与 vscode mode，并把 Claude normal 的当前 live 命令面收口到新的 backend-aware 主链：`codex normal` 继续以 `workspace` 命令族作为主展示壳，`claude normal` 当前 live 实现把工作会话主链显式收口到 `/new`、`/list`、`/use`、`/detach`，把 `常用工具` 收口到 `/history`，并在 `参数设置` 新增 `/claudeprofile`；同时 `/list` `/use` / target picker / workspace recency 全部只按当前 backend 过滤，且不再因为 surface/instance `ClaudeProfileID` 不同而隐藏 Claude workspace/session 候选。`Claude <-> Codex normal` 的 backend 互切现在统一锚定当前工作区目录：只要切换前已有当前 workspace，surface 就会保留这份 workspace claim，并优先 attach 目标 backend 下同 workspace 的在线实例；若目标 backend 当前还没有同 workspace 实例，则会直接启动 fresh managed headless，并保留“进入新会话待命”的意图。进入 Claude workspace 时，surface 还会按 `workspace+profile` 快照恢复 `reasoning / plan / access`，离开该 workspace/profile 时再把这三项写回独立持久化 store，而 `/status` 只读投影当前 profile。2026-04-30 的新变化是：Claude normal 现在也打通了 exact-thread 精确追回闭环，persisted thread lookup、detached `/use`、normal startup resume 与 managed headless exact-thread continuation 全部按 backend 分流；同时 daemon startup recovery 已收口成单一 normal-mode 主链，不再依赖旧的第二张 headless restore runtime 表或独立 hint 投影；Claude runtime 会把可恢复旧 session 显式转成 launch-time `--resume <session_id>`，而普通新 prompt 不会再因为任意目标 thread id 被误判成 session switch。其余能力仍保持此前基线：normal mode 的被动恢复入口（attach unbound、`selected_thread_lost`、`thread_claim_lost`）统一回到“锁定当前工作区”的 target picker，不再回退旧 scoped selection prompt；VS Code `/list` / `/use` / `/useall` 继续走结构化实例/线程卡，其中线程选择统一成当前实例内的 dropdown，并隐藏不可切换会话、改用 plain-text 提示说明。surface-level backend seam 也已正式落成真实状态：normal mode 区分 `codex` / `claude` 两个 backend，workspace defaults、surface resume 与 detached catalog context 都按 backend 分区，`/mode` 的底层语义收口成 `codex|claude|vscode`，其中 `normal` 仍作为 `codex` 的兼容 alias。另一个新变化是把上游 runtime 问题自动继续从 `autowhip` 中拆成独立 `autocontinue` overlay：它由 orchestrator 本地 codex/gateway error-family policy 驱动，拥有自己的 queue lane、reply anchor、tail-only 状态卡与 backoff，不再和“正常结束后继续催活”混用；request gate 现在还补上了 `item/tool/call` 的最小 fail-closed 分支：relay / Feishu / headless 会展示只读 `tool_callback` 提示，并立即自动回写 unsupported 结构化结果，避免 tool 在中途 silent hang；同时 detached-branch 产品入口已经正式接上：普通文本里的 `[什么？]` / `[耸肩摊手]` 会分别触发 `fork_ephemeral` / `start_ephemeral`，统一复用 `keep_surface_selection`，且不会再让 detour turn 污染当前 surface 默认 thread；review mode 的 detached review session 也已接入同一条远端状态机：review thread 会带显式 `source=review` / parent-thread 元数据，surface 会在不改绑当前选中 thread 的前提下记录 `ReviewSession` runtime，并把后续审阅文本继续路由到 review thread；与此同时，普通 attach/list/use 候选现在会显式排除 `source=review` 会话，不再把 detached review thread 混进 merged thread list、current-instance dropdown 或 workspace recency。本轮还把 `process.child.restart` 收口成“两段式 restart 合同”：`ack` 只代表新 child 已接管，thread restore 结果改由独立 outcome event 回传，daemon 会对 `/bendtomywill` 与 standalone Codex upgrade 统一等待最终 outcome，因此 late restore 不会再把 patch / upgrade 误判成已失败或已完成。注意：2026-04-29 新拍板的下一轮 Claude MVP 产品边界不再由本文定义，而改以 [Claude Backend Integration Plan](../inprogress/claude-backend-integration-plan.md) 第 `7.6` / `12.1` 节为准；本文仍只记录当前 live 实现。
...
| 代号 | 条件 | 用户语义 |
| --- | --- | --- |
| `M0 CodexNormal` | `ProductMode=normal`，`Backend=codex` | 新 surface 默认值；当前会开启 workspace claim 仲裁，并把已占用 workspace 投影到 `/status` |
| `M1 ClaudeNormal` | `ProductMode=normal`，`Backend=claude` | 仍走 workspace-first normal-mode 语义，但 workspace defaults、surface resume 与 detached catalog context 都按 Claude backend 分区；surface 还会额外携带当前 `ClaudeProfileID`，并按 `workspace+profile` 恢复 `reasoning / plan / access`；不进入 VS Code 语义，但已经共享 normal exact-thread 恢复主链，并在需要时通过 Claude 原生 `--resume` 恢复旧 session |
| `M2 VSCode` | `ProductMode=vscode`，`Backend=codex` | 只能显式 `/mode vscode` 进入；当前不参与 workspace claim，仍保留既有 instance/thread-first 路由语义 |
...
2. `ProductMode` / `Backend` 当前已经进入 daemon 级 `surface resume state`：
   ...
   4. `normal` mode surface 随后会按 persisted resume target 继续尝试恢复：
      ...
      7. 这条 managed-headless exact-thread continuation 当前仍按 backend 生效：Codex 继续走 sqlite/persisted-thread + child-restore 语义；Claude 会把同 backend persisted session metadata 转成 launch-time `ResumeThreadID`，最终由 wrapper 用 `claude --resume <session_id>` 恢复旧 session。
   ...
   5. `vscode` mode surface 会按 persisted `ResumeInstanceID` 继续尝试恢复：
      ...
      5. `vscode` mode 不会参与 managed-headless exact-thread continuation；`surface resume state` 在 load/write 时也会把所有非 `normal` mode entry 的 `ResumeHeadless` 强制归零，因此 `vscode` surface 不会保留可继续触发这条恢复分支的持久化目标。
...
6. 当前工作会话命令已经按 mode 分流：
   ...
   2. `claude normal` 的 visible MVP 继续复用同一套命令目录壳：工作会话主链显式开放 `/new`、`/list`、`/use`、`/detach`，`常用工具` 收口为 `/history`；`/review` 与 `/bendtomywill` 当前已经退出 Claude 主展示面，回到 hidden + reject；`workspace*` 与 `/useall` 继续 hidden + reject。
...
8. `PlanMode` 当前也是 surface 级偏好：
   ...
   4. `/detach`、`/new`、`/use`、`/mode normal|codex|claude|vscode` 不会顺手清掉它；daemon 重启后，latent surface 会从 `surface resume state` 恢复之前的 `PlanMode`。
   4.1. 但在 `claude normal` 下，`PlanMode` 不再只是“纯 surface 全局值”：
      1. 进入某个 Claude workspace 时，会按 `workspace + ClaudeProfileID` 恢复最近一次的 `PlanMode / ReasoningEffort / AccessMode`。
      2. 离开该 workspace 或切走该 profile 前，会把当前这三项写回独立的 `workspace+profile` 持久化 store。
      3. 若目标 `workspace+profile` 没有快照，则会恢复成空 override + `PlanMode=off`，不会沿用别的 workspace/profile 残留值。
      4. `Model` override 明确不在这套快照里；Claude workspace/profile 恢复时会主动清掉临时模型覆盖，不把它当作可持久化热改能力。
...
1. `R0 Detached` 现在允许存在一种 daemon materialize 出来的 latent surface：
   ...
   6. 若该 surface 的 `surface resume state` 里仍带有 `ResumeHeadless=true` 的 concrete thread-restore 目标，daemon 会在同一条 normal-mode recovery 主链里先尝试 exact visible attach；只有 visible 路径没恢复成功时，才继续进入 managed-headless exact-thread continuation；`vscode` mode 不会进入这条分支。这里的 continuation 已按 backend 生效，不再只限 Codex。
...
| 代号 | 条件 | 当前实现语义 |
| --- | --- | --- |
| `V0 None` | `ReviewSession == nil`、`Phase != active`，或 `SelectedThreadID != ParentThreadID` | 当前 surface 不在 review session 里 |
| `V1 Active` | `ReviewSession.Phase=active`，且 `ParentThreadID`、`ReviewThreadID`、`AttachedInstanceID` 都非空，同时 `SelectedThreadID == ParentThreadID` | 当前 surface 正处于 detached review session；普通文本默认续发到 review thread，但 surface 仍保持 parent thread 选择 |
...
3. 当前激活条件不是“点了某个前台按钮”，而是更底层的 runtime 事实：
   ...
   3. surface 当前仍选中 `ParentThreadID`
4. `V1 Active` 不会把 surface route 从 `pinned/follow` 改成新的 route 值；review session 只是额外改写“普通文本该发到哪个 execution thread”。若用户进入 `/new` 的 `new_thread_ready`，空闲 review session 会先被清掉，避免首条新会话文本继续落到 review thread。
...
### 4.1 codex normal `workspace` 命令族先打开工作会话页面或业务卡，confirm 后再改 route

当前 `codex normal` 的工作会话主展示已经切到 `workspace` 命令族：bare `/workspace` / `/workspace new` 负责父页导航，`/workspace list` / `/workspace new dir` / `/workspace new git` / `/workspace new worktree` 负责四张独立业务卡；`/list` / `/use` / `/useall` 只保留 alias，并在 `codex normal` 下汇合到 `/workspace list`。
...
### 4.1.1 当前 live Claude 命令面继续复用同一工作会话壳，但已回退 review / patch 主展示入口

当前 `claude normal` 不再沿用 `codex normal` 的 `workspace` 主入口投影，但会继续复用同一套 workspace/session route contract 与 target picker 壳。
...
1. `ResolveFeishuCommandStrategy()`、`ResolveFeishuCommandDisplayProfileForContext()` 与 display-group projection 当前会把 Claude visible MVP 固定为：
   1. `current_work` 只保留 `/stop`、`/new`、`/status`、`/detach`。
   2. `switch_target` 只保留 `/list`、`/use`。
   3. `send_settings` 当前显式开放 `/reasoning`、`/model`、`/access`、`/verbose` 与 `/claudeprofile`。
   4. `common_tools` 当前只显式开放 `/history`。
   5. `/review`、`/bendtomywill`、`workspace*`、`/useall`、`/plan` 与 `/autocontinue` 继续 hidden + reject，不会在 Claude mode 下伪装成可用入口。
2. `/list` 与 `/use` 当前继续复用 normal-mode target picker / workspace-session shell，不另造一套 Claude 专用卡。
3. 但 target picker 的 workspace/session 候选已经只按当前 backend 过滤：
   1. `mergedThreadViews()` 只合并当前 normal-mode backend 的在线实例。
   ...
   3. `workspaceOnlineInstancesForSurfaceBackend()`、`resolveWorkspaceAttachInstance()` 与 `reusableManagedHeadless()` 不再按 `ClaudeProfileID` 缩窄同 backend 候选。
   4. Claude normal 不再把 persisted Codex recent threads/workspaces 混进 `/list` / `/use` 候选。
   5. detached `/use`、normal startup resume 与 concrete headless restore 现在也会按当前 backend 读取 persisted exact-thread metadata；Claude 可以消费 persisted Claude session，但不会误吃 Codex sqlite thread。
...
6. `/claudeprofile` 当前是 Claude normal 专属的 surface 级切换入口：
   ...
   5. 当前 workspace 已占用时，切 profile 会先 detach-like 清理旧 runtime，再按目标 `workspace+profile` 快照恢复 `PlanMode / ReasoningEffort / AccessMode`，最后直接走 fresh headless prepare；若原 surface 处在 `/new` 待命态，`PrepareNewThread` 也会一并保留。
6. `ClaudeProfileID` 当前也是 surface runtime carrier：
...
7. daemon 在 fresh/preselected headless launch 时会把 `CODEX_REMOTE_INSTANCE_BACKEND=<surface backend>` 注入环境；若当前 backend 是 `claude`，还会把当前 `ClaudeProfileID` 一并下发到 headless command，因此 Claude headless 会按正确 profile 启动，而不是默回 Codex 或默回别的 Claude profile。

### 4.1.2 vscode mode `/list` 先选 instance，并显式投影“当前实例”
...
3. 切换时一定先做 detach-like 清理；大多数目标会进入 detached 态，但若切到 `claude normal` 且切换前已有当前 workspace，则会保留该 workspace claim，并立即进入“attach 已在线 Claude instance”或“启动 fresh Claude headless”的后续链路，而不是停在 detached idle。
4. 若切换前存在 `PendingHeadless` 或 `surface resume state` 里仍带着 headless 恢复目标，会一并 kill / clear，避免 mode 切完以后又被后台恢复拉回 headless。
5. `vscode` surface 不参与 managed-headless exact-thread continuation；而且 `surface resume state` 会把非 `normal` mode 的 `ResumeHeadless` 硬归零，避免 daemon 重启后从持久化状态重新长出这条恢复入口。
6. 当前 mode 会跨 daemon 重启保留：
   1. startup 会先恢复 latent surface、`ProductMode` 与 `Verbosity`
   2. `normal` mode 会继续按 persisted target 尝试自动恢复：workspace-owned route 直恢复 workspace intent；thread target 先 exact visible attach；`ResumeHeadless=true` 时再继续 exact-thread continuation
...
7. 切换当前已经会改变 `/list` 的主交互语义：
   1. `codex normal` 与 `claude normal` 下 `/list` 都是 workspace chooser。
...
2. 二级分组顺序稳定，但组内可见命令会按当前 `product mode + menu stage` 做 display projection：
   1. `codex normal` 的菜单首页点击 `工作会话` 时，不再进入旧的命令分组页，而是直接打开 bare `/workspace` 父页。
   ...
   4. `codex normal` 下 `/list`、`/use`、`/useall`、`/detach` 不再作为主展示菜单项，但 alias / parser 兼容仍保留，并分别汇合到 `/workspace list` 与 `/workspace detach`。
   5. `claude normal` 下 `工作会话` 分组当前直接显示 `/new`、`/status`、`/detach`，`switch_target` 分组直接显示 `/list`、`/use`；`workspace*`、`/useall`、`/review` 与 `/bendtomywill` 不再出现在主展示菜单里。
   6. `vscode mode` 的 `工作会话` 仍分别显示 `/list`、`/use`、`/useall`。
   7. `normal mode` 不展示 `/follow`；`vscode mode` 才展示 `/follow`。
   8. `/new` 只在 `normal` working 可见；`/status` 当前在 `基本命令`，`/history` 在 `常用工具`，其中 `/status` 与 `/history` 在 normal / vscode 都可见；对 `claude normal`，`/detach` 当前也进入 `工作会话` 分组，而 `/review` / `/bendtomywill` 已退出主展示面。
3. `/help` 当前也复用同一套 display projection：
   1. `codex normal` 下帮助文本里的主展示入口已经切到 `workspace` 命令族：`/workspace`、`/workspace list`、`/workspace new`、`/workspace new dir`、`/workspace new git`、`/workspace new worktree`、`/workspace detach`。
   2. `claude normal` 下帮助文本的主展示入口当前会保留 `/new`、`/list`、`/use`、`/detach` 这组 visible MVP 会话主链，并在 `常用工具` 只展示 `/history`；`/review`、`/bendtomywill`、`workspace*` 与 `/useall` 仍只存在于 parser / reject 兼容层，不作为主展示帮助入口。
...
23. detached review session 当前复用同一套“execution thread 与 surface selected thread 分离”的承接方式，但语义比 detour 更强：
   ...
   3. surface 自己的 `SelectedThreadID` 仍保持 parent thread；因此 review turn 不会污染后续普通 attach/resume 默认目标。`ReviewSession` 只有在 `SelectedThreadID == ParentThreadID` 时才被视为 active，`/new` 进入 `new_thread_ready` 会清掉空闲 review session
...
7. 如果该 surface 的 `surface resume state` 仍保留 `ResumeHeadless=true` 的 concrete thread-restore 目标，hard disconnect 回到 `R0 Detached` 后会重新进入同一条 normal recovery 主链里的 exact-thread continuation 判定。
...
| `/new` | 拒绝 | `normal`: 允许；`vscode`: 拒绝 | `normal`: 允许；若存在 compact/steer/queued/dispatching/running 或运行中的 review turn 则拒绝；空闲 review session 会被清掉；`vscode`: 拒绝 | 拒绝 | 拒绝 | 允许；若首条消息已 dispatching/running 则拒绝；空闲 review session 会被清掉 |
...
| `/detach` | 允许但通常只提示已 detached；`codex normal` 主展示命令仍是 `/workspace detach`，`claude normal` 主展示命令已切到 `/detach` | 允许；`codex normal` 主展示命令仍是 `/workspace detach`，`claude normal` 主展示命令已切到 `/detach` | 允许；`codex normal` 主展示命令仍是 `/workspace detach`，`claude normal` 主展示命令已切到 `/detach` | 允许；`codex normal` 主展示命令仍是 `/workspace detach`，`claude normal` 主展示命令已切到 `/detach` | 允许；`codex normal` 主展示命令仍是 `/workspace detach`，`claude normal` 主展示命令已切到 `/detach` | 允许；dispatching/running 时走 abandoning；`codex normal` 主展示命令仍是 `/workspace detach`，`claude normal` 主展示命令已切到 `/detach` |
