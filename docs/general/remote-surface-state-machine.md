...
6. 当前工作会话命令已经按 mode 分流：
   ...
   2. `claude` 的 visible MVP 继续复用同一套命令目录壳：工作会话主链显式开放 `/new`、`/list`、`/use`、`/detach`，`常用工具` 收口为 `/history`；`/review` 与 `/bendtomywill` 当前已经退出 Claude 主展示面，回到 hidden + reject；`workspace*` 与 `/useall` 继续 hidden + reject。
...
8. `PlanMode` 当前也是 surface 级偏好：
   ...
   4.1. 但在 `claude` 模式下，`PlanMode` 不再只是“纯 surface 全局值”：
...
3. `/help` 当前也复用同一套 display projection：
   ...
   2. `claude` 下帮助文本的主展示入口当前会保留 `/new`、`/list`、`/use`、`/detach` 这组 visible MVP 会话主链，并在 `常用工具` 只展示 `/history`；`/review`、`/bendtomywill`、`workspace*` 与 `/useall` 仍只存在于 parser / reject 兼容层，不作为主展示帮助入口。
