...
15. [internal/core/orchestrator/service_recovery.go](../../internal/core/orchestrator/service_recovery.go)
...
8. daemon 在 fresh/preselected headless launch 时，会只消费 orchestrator 已冻结下来的 headless launch contract：
   ...
   3. 实际注入环境时仍会落 `CODEX_REMOTE_INSTANCE_BACKEND=<frozen backend>`；若当前 frozen backend 是 `claude`，还会把当前 `ClaudeProfileID` 与 `CLAUDE_CODE_EFFORT_LEVEL` 一并下发到 headless command，因此 Claude headless 会按正确 profile 与推理强度启动，而不是默回 Codex、默回别的 Claude profile，或继续沿用旧 reasoning。
...
10. Claude `/reasoning` 当前也已并入统一的 dispatch 前 preflight，而不再只是 surface 上的 UI 值：
   ...
   2. 普通 queue dispatch、auto-continue 与 review apply 都会在真正 `prompt.send` 前，用各自 frozen override 生成 `desired headless launch contract`，再与 wrapper hello 上报的 observed runtime contract 比较。
   3. 合同一致时直接发送；不一致时会进入 `PendingHeadless(Purpose=prompt_dispatch_restart)`，先显式 kill 当前 managed headless，再 fresh-start 匹配 reasoning 的新实例。
   4. 新实例连回后只做最小 reattach，不会重置 queue、review session 或 auto-continue runtime；统一 dispatch owner 会继续把原始 prompt 发出去。
   5. `/access` 与 `/plan` 仍保留动态 permission-mode 通道，不并入这条 restart-only 合同。
