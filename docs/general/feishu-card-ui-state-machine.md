...
| stamped `/menu ... -> /help` / `/status` | `mixed` | 当前不再走 append-only 帮助卡或提交态锚点；点击后 daemon 会把 handler 产出的首个结果卡（帮助目录或 snapshot 状态卡）直接作为 `ReplaceCurrentCard`，让原菜单卡继续承担结果承载。当前 `/help` 位于 `maintenance` 分组，`/status` 位于 `current_work` 分组。纯文本 `/help` / `/status` 仍保持 append-only |
...
- `/menu` 分组内命令的阶段可见性当前已收束到统一策略：
  ...
  - `/status` 当前不额外分阶段，normal / vscode 里都默认可见
  - `/history`、`/sendfile`、`/cron`、`/autowhip` 当前不额外分阶段，normal / vscode 里都默认可见；真正能否拿到历史由当前 route 是否能解析出 thread 决定
  - 其余命令默认可见
