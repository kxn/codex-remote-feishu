...
3. `first_result_card` 路径
  ...
  - 普通 final card 上的 `进入审阅` 当前不再命中 first-result replacement，而是保持 append-only：daemon 会保留源 final card，并额外发送一张“正在进入审阅”提示卡
...
- [internal/app/daemon/app_review_mode_test.go](../../internal/app/daemon/app_review_mode_test.go)
  - 锁定普通 final card 上的 `进入审阅` 保持 append-only、不覆盖源 final card；review session final card 的 `放弃审阅` / `按审阅意见继续修改` 继续复用 stamped result replacement
