...
| bare `/review uncommitted` / review final card `Review 待提交内容` / `放弃审阅` / `按审阅意见继续修改` | `mixed` | bare `/review uncommitted`、常用工具菜单入口与普通 final card 上的 `Review 待提交内容` 当前都汇合到同一 detached review owner：owner 会先按当前上下文或 source final card 恢复 thread/cwd，再以 Git worktree dirty 状态做最终校验。按钮 payload 仍由 Feishu UI 生成 `page_action + daemon_lifecycle_id`，并继续复用 freshness 与 old-card reject；但当前卡语义已经分开：普通 final card 上的 `Review 待提交内容` 保持 append-only，只额外发送一张“正在进入审阅”提示卡，不再 patch 掉源 final card；review session final card 上的 `放弃审阅` / `按审阅意见继续修改` 仍复用 stamped first-result replacement，在当前审阅结果卡上同位收口。真正的 detached review session 启动、review runtime 清理，以及把审阅结果带回 parent thread 继续修改，仍由 orchestrator 承接。普通 final card 只有在对应 thread cwd 落在 Git repo/worktree 内且存在未提交内容时才会追加 `Review 待提交内容`；review session 内的 final card 只保留 `放弃审阅` / `按审阅意见继续修改` 两个显式出口 |
...
- [internal/app/daemon/app_review_mode_test.go](../../internal/app/daemon/app_review_mode_test.go)
  - 锁定普通 final card 上的 `Review 待提交内容` 保持 append-only、不覆盖源 final card，并按 Git dirty 状态而不是 file summary 决定显隐；同时锁定 `/review uncommitted` 会走同一 detached review owner，review session final card 的 `放弃审阅` / `按审阅意见继续修改` 继续复用 stamped result replacement
