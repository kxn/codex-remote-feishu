...
- `/menu` 分组内命令的阶段可见性当前已收束到统一策略：
  ...
  - `claude normal` 的 `send_settings` 分组当前只保留 `/reasoning`、`/model`、`/access`、`/verbose`、`/claudeprofile`
...
- [internal/core/control/feishu_command_strategy_test.go](../../internal/core/control/feishu_command_strategy_test.go)
  - 锁定 Claude visible MVP 的命令矩阵：`/new`、`/list`、`/use`、`/review`、`/bendtomywill` 现在是 visible + allow approximation，`/steerall` 继续显式 reject
- [internal/core/control/feishu_command_display_resolver_test.go](../../internal/core/control/feishu_command_display_resolver_test.go)
  - 锁定 Claude `current_work` / `switch_target` / `common_tools` 的 help/menu projection：`/stop`、`/new`、`/status`，`/list`、`/use`，以及 `/review`、`/bendtomywill`、`/history`
