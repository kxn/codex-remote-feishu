...
- `daemon`
  ...
  - `ResolveFeishuFrontstageActionContract(action)` 里的 `ContinuationDaemonCommand` 与 `FollowupPolicy` 现在也优先从 `FeishuCommandBinding` 读取：`/debug`、`/cron`、`/upgrade`、`/repair`、`/vscode-migrate` 的首结果卡续接 daemon command，以及 `/help`、`/status`、`/stop`、`/new`、`/follow`、`/detach` 这组命令的 followup drop 策略，不再分别散落在 lifecycle switch 里
...
| `/repair` | `daemon_command` | `/repair` 和 `/repair daemon` 都不打开 page，也不替换当前菜单/参数/根页卡。文本、bot menu 或 stamped card callback 都会立即 ack 并 append 一条 `Repair` started notice；默认 `/repair` 后台执行当前 surface 修复，完成后继续 append success/partial notice；`/repair daemon` 只尝试通过托管 install lifecycle manager 重启当前 daemon，无法确认托管时 append 失败提示。它允许出现在 `/help` 与 `/menu maintenance`，但不进入 recommended Feishu app manifest 菜单，避免要求后台菜单配置同步。 |
...
- command/config cards 当前已分为两条 read-model 边界：
  - `/menu`、bare config cards、bare `/cron` `/upgrade` `/restart` `/debug` 根页当前跨边界统一携带 `control.FeishuPageView`（`UIEventFeishuPageView`）；`/repair` 是 immediate daemon command，不产出 root page
...
- `/menu` 分组内命令的阶段可见性当前已收束到统一策略：
  ...
  - `maintenance` 分组当前的 canonical menu-visible 命令为 `/mode`、`/upgrade`、`/repair`、`/restart`、`/debug`、`/help`
...
- `/repair` 当前是 immediate daemon command，不分 page / execute：
  - 它即使来自 stamped 当前卡也不走同步 replace，只 append `Repair` started notice
  - 后续结果仍 append-only；其中 provider child 修复复用 `/restart child` 的安全重启合同，但 busy/unsupported 时只跳过 provider 侧并报告原因
  - `/repair daemon` 也是 append-only immediate daemon command；它不会复用默认 `/repair` 的 bot runtime / provider child / managed headless 修复链路，只读取 install-state 并在 `systemd_user` 托管时发起 daemon restart
...
- bare `/upgrade`、bare `/restart`、bare `/debug`、bare `/cron`、bare `/vscode-migrate` 当前已经退出 bare continuation 与提交态锚点；它们统一改成 page contract 根页/子页模型，stamped current-card callback 会直接把下一张 page 同位替回当前卡，非法参数也继续留在当前页内报错。`/repair` 不属于这组 page command；它是 append-only immediate daemon command。
...
- `/cron`、`/upgrade`、`/restart`、`/debug` 的 stamped 菜单入口与 stamped page callback，当前属于根页 / 子页 / 非法参数回显的路径都会直接把下一张 page 或结果卡同位替回当前卡，不再先外跳 append 一张独立状态/输入卡；其中 `/restart child` 仍然保持立即执行 + append-only。`/repair` 始终保持立即执行 + append-only。
