...
| `/repair` | `daemon_command` | `/repair` 不打开 page，也不替换当前菜单/参数/根页卡。文本、bot menu 或 stamped card callback 都会立即 ack 并 append 一条 `Repair` started notice，后台执行当前 surface 修复；完成后继续 append success/partial notice。它允许出现在 `/help` 与 `/menu maintenance`，但不进入 recommended Feishu app manifest 菜单，避免要求后台菜单配置同步。 |
