...
> Type: `implemented`
> Updated: `2026-04-06`
> Summary: 标记为已实现文档并迁移到 `docs/implemented`，记录当前 Markdown 预览能力的落地边界。
...
- `/data/dl/fschannel/docs/general/feishu-product-design.md`
...
- ``[绝对路径](/data/dl/fschannel/README.md)``
...
```text
~/.local/state/codex-remote/feishu-md-preview.json
```
...
当前实现还没有自动 GC。下面规则仍属于后续工程化建议，不应理解为“现已上线行为”。

### 11.2 推荐规则

V1 建议同时使用两类规则：

- 每个 `absolute_path + surface` 只保留最近 `N` 个版本
- 删除超过 `TTL` 且最近未被引用的版本
...
飞书删除后文件进入回收站，不是立即硬删除，这对误删恢复更安全。

### 11.4 GC 触发方式

推荐两种触发：

- 惰性清理：每次新上传成功后，顺手清理当前 surface 下超额旧版本
- 定时清理：daemon 定时扫描索引并批量回收过期版本
