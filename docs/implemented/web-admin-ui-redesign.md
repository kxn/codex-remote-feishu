...
> Type: `implemented`
> Updated: `2026-04-29`
> Summary: 当前 admin 页面以 `v1.7.0` 为基线，默认主界面只保留机器人管理、系统集成、Claude 配置、存储管理四块；新增机器人流程与 setup 保持相同边界，并支持权限强制跳过。
...
- 当前任务：查看机器人状态、增加机器人、管理 Claude 配置、处理存储占用
- 允许展示：机器人状态、测试入口、机器集成状态、Claude 配置、存储状态和清理入口
...
`Claude 配置` 是当前相对 `v1.7.0` 唯一新增的默认主界面区块。

它默认直接放在 admin 主界面，不额外扩展新的页面骨架。

## 7. 当前结论

- admin 不是新的 setup 页面
- setup/admin 的主界面结构都以 `v1.7.0` 为基线
- 当前唯一接受的新默认主界面内容是 `Claude 配置`
