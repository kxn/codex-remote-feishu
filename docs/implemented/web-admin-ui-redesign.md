...
- 当前任务：查看机器人状态、增加机器人、管理 Claude 配置、管理 Codex Profile、处理存储占用
- 允许展示：机器人状态、测试入口、机器集成状态、Claude 配置、Codex Profile、存储状态和清理入口
...
`Codex Profile` 也直接放在 admin 主界面，和 `Claude 配置` 同级。
...
- 当前接受的新默认主界面新增内容是 `Claude 配置` 与 `Codex Profile`
