...
> Type: `draft`
> Updated: `2026-05-01`
> Summary: 固定共享 home 前提下的 Codex Provider 管理边界：provider 与密钥由本仓库自行管理，启动时用 `-c` 覆盖与子进程环境变量注入，不持久改写共享 `~/.codex/config.toml` 或 `auth.json`。
...
但当前还没有一条完整产品路径，允许用户在 Web setup/admin 中：
...
本设计的目标，是把这条路径做成与当前 `Claude 配置` / `Cloud 配置切换` 相同风格的产品能力，同时继续遵守 `v1.7.0` setup/admin 页面基线，不额外扩页面骨架，也不把内部技术语义暴露到主界面。
...
切换 provider 的正式语义与当前 Cloud 配置切换保持一致：
...
1. 名称必填
2. `baseURL` 必填
3. 归一化后 id 不能等于 `default`
4. 归一化后 id 不能等于 `openai`
5. 名称去空白并忽略大小写后不能等价于 `openai`
...
- 某些 OpenAI-compatible 端点可能由本地代理或网关完成认证
- 当 API key 为空时，不应传 `env_key` 覆盖
...
当且仅当当前选择的 provider 保存了 API key 时：
