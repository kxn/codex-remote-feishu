...
4. 查询参数 `user_id_type` 支持 `open_id`、`user_id`、`union_id`。为了避免跨 app open_id 不一致，V1 建议优先用 `user_id` 判断，但这要求申请 `contact:user.employee_id:readonly` 字段权限；如果权限缺失，则只能退回同 app 的 `open_id` 判断，并把“多 app 共享群 workspace”的能力降级。
...
结合当前证据，不能把跨 app 稳定的 `OpenChatID` 当作已证明前提。V1 若要支持“同一群内多个机器人共享 workspace”，需要分两种范围：

1. 同一个 gateway / app 内：可以继续用当前 `feishu:<gatewayID>:chat:<chatID>` 识别 surface，并在 room owner 内允许该 app 的多个 bot / surface 共享 workspace。
2. 跨不同 app / gateway：不能只靠 `chatID` 自动合并为同一个 room。除非后续通过实测或官方文档证明 `chat_id` 跨 app 稳定，或者引入显式绑定 / 管理员确认的 room linkage，否则应默认隔离。

如果后续确实要跨 app 共享，需要额外设计一个显式 room linkage：由群管理员在同一群内分别对多个 app 完成绑定确认，系统记录 `tenant_key + app_id + chat_id` 到同一内部 `RoomContextID` 的映射，并在管理员身份、workspace claim、重置广播上全部按内部 room id 操作。

## 8. 当前主要风险

1. 群 room key 不能假设跨 app 稳定；Feishu `OpenChatID` / `chat_id` 在多 app 场景下未找到官方稳定性承诺，默认不能自动合并。
...
6. Codex 底层虽支持单实例多 active turn，但当前仓库单槽 active turn 模型会导致所有权、stop / steer、请求审批和飞书最终卡投递风险；不能半改。
7. 飞书群管理员身份依赖补充 API，必须处理权限缺失、机器人不在群、外部群、接口限流和缓存过期。
...
6. 保存 room identity proof：至少包含 `tenant_key`、`gatewayID` / `app_id`、`chat_id`，跨 app linkage 只能通过显式绑定产生。
...
4. Feishu `OpenChatID` 是否跨不同 app / bot 稳定：未找到官方明确承诺，不能作为 V1 自动跨 app 合并 room 的依据。
...
V1 不支持本仓库内的单实例并发多 session。推荐实现为：
...
6. 跨不同 Feishu app / gateway 的“同一群”默认不自动合并，除非后续新增显式管理员绑定。
