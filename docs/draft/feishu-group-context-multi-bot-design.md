...
这说明当前环境里 `chat_id` 在两个 app / gateway 下实测稳定，但这仍不是官方长期合同。产品设计可以把它作为强信号使用，但跨 app 自动合并 room 仍需要防御式设计或显式绑定预案。
...
2. 群 room key 不能假设跨 app 稳定；Feishu `OpenChatID` / `chat_id` 在多 app 场景下未找到官方稳定性承诺，默认不能自动合并。
...
7. 跨不同 Feishu app / gateway 的“同一群”默认不自动合并，除非后续新增显式管理员绑定。
