...
1. 哪些 scopes / events / callbacks 属于哪个功能
2. 哪些项是核心阻塞项，哪些项可降级
3. 哪些项在 UI 上应如何提示功能降级
...
这三类值属于产品固定策略，不需要进入 manifest。

如果不先把 manifest 收口成“功能声明 + 降级语义”的目标模型，自动配置就只能继续把功能边界写死在流程代码里，后面一定再次发散。
...
1. 只支持飞书
2. 不支持 Lark
3. 不为其他平台预留额外的目标状态或兼容描述

结论：

1. setup / admin 的自动配置实现直接按 Feishu 能力建模。
2. 当前文档不再讨论其他平台的能力探测、显隐或降级路径。
3. 如果未来要支持其他平台，应单独开新设计，不在这份文档里预留半套抽象。
...
### 5.1 需要把 manifest 升级成“功能声明目标状态”

现有 `DefaultManifest()` 需要从“人工 checklist 数据”扩成“自动配置里关于功能清单和降级语义的 source of truth”。
...
2. 读取当前 scopes 授权状态
3. 读取当前在线版本 / 待发布版本信息
4. 基于 manifest 清单和后端固定策略计算“目标状态 vs 当前状态”的差异
5. 如有差异，先执行一次 `config PATCH`
6. 如固定能力基线有差异，再执行一次 `ability PATCH`
7. 重新读取 scopes 与版本信息
8. 判断是否需要 `publish`
...
3. `config PATCH` 中的订阅方式、回调方式、“不使用 request URL”等固定策略，统一由后端常量写入，不从 manifest 读取。
...
1. setup 只为飞书 app 提供这套自动配置
2. admin 只为飞书 app 提供这套自动配置
3. 不为其他平台增加兼容分支
...
1. 公开读接口不完整，必须接受一部分字段只能做弱校验。
2. 发布前无法精确预测租户管理员审核要求，UI 必须诚实表达不确定性。
3. 如果不先把 manifest 收口成“功能声明 + 降级语义”，setup 和 admin 很快会再次分叉。
