...
> Type: `draft`
> Updated: `2026-08-16`
> Summary: 为排队消息增加 `APPLAUSE` reaction 触发 Codex `thread/shellCommand` 注入，并收窄为同机临时文件载荷、完整清理和队列互斥语义。
...
新增 `APPLAUSE` 入口用于另一种动作：把排队消息的文本内容包装成 user shell command，通过 Codex app-server 的 `thread/shellCommand` 注入当前正在执行的 turn。它不是 `turn.steer` 的别名，也不替换现有点赞 steering。
...
- 不在 v1 为图片单独实现 shell command 注入。带绑定图片的 queue item 不应只注入文字而静默丢失图片。
...
临时文件必须由可信的 runtime 组件在本功能专用临时根目录下按 `runtimeID` 分目录，以排他方式创建，随机文件名、权限 `0600`，并拒绝符号链接和用户提供的路径。正常路径下，shell command 完成、明确拒绝或失败后立即清理；结果未知时保留带 TTL 的临时文件记录，不重试命令。daemon/wrapper 启动时必须只扫描本功能专用根目录，清理属于旧 runtime 且已过 TTL 的 runtime 目录及其文件，处理上次崩溃遗留物；不能用宽泛的系统临时目录清理代替。文件名、路径和内容不得写入普通日志。由于 `thread/shellCommand` 是 full access，文件 staging 不能依赖 Codex sandbox 来限制读取范围。
...
- queue item 有可注入的文本，且没有未处理的绑定图片。

图片消息、已 dispatching/running/completed 的消息、其他 surface/thread 的消息、没有 active turn 的消息均不触发。
...
| 非主文本、无 active turn、错误 thread、图片 item、backend 不支持 | 不变 | 不添加 | 静默 no-op |
...
- `ShellCommand.Text`：结构化的待注入文本；不要让 core 直接暴露任意 shell 字符串。
- `ShellCommand.PayloadRef`：同机临时文件引用；必须绑定 runtime、thread、command ID 和 TTL，不能接受用户提供的任意路径。

wrapper/adapter 在同机专用临时目录 staging 文件后，只生成固定的文件读取 command，再发送 native `thread/shellCommand`。优先等待 `UserShell` command execution 完成后清理文件；仅收到 accepted ack 不足以提前删除。command ack 绑定到独立 `pendingShellCommandBinding`，不进入 `pendingSteerBinding` 或普通 remote turn binding。
...
2. Codex adapter/wrapper：实现 native `thread/shellCommand` 翻译、active-turn guard、同机专用目录文件 staging、正常/异常/重启清理、平台 quoting、native error 归一化和 user-shell event 关联。
...
- 无 active turn、错误 frozen thread、compact/request gate、图片 item、OpenCode/Claude 或无 capability：item 不变且静默。
...
- payload-file 模式下，shell command 只包含安全转义后的临时文件路径，读取结果与原始 UTF-8 内容一致；文件完成、拒绝、失败和 TTL 到期都会清理。
...
- 载荷：只写同机 execution environment 的 UTF-8 临时文件，shell command 只读取文件；不提供 inline/`echo` fallback。
