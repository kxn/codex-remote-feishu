...
> Type: `draft`
> Updated: `2026-05-13`
> Summary: 收敛工作区级 `Autopilot.md` 的编译、加载、事件分流、工作区开关，以及对现有 `/cron`、`autocontinue`、`autowhip` 的映射边界。
...
- `/autopilot`
  - 纯 bare command。
  - 只负责打开 Autopilot 菜单，不直接承载状态展示或其他业务语义。
- `/autopilot list`
  - 列出当前工作区已经识别并可用的 Autopilot 项。
  - 这里的“项”以编译结果中的 entry 为准，而不是重新渲染原始 `Autopilot.md`。
- `/autopilot load`
...
`/autopilot` 与 `/autopilot status` 不再等价。

原因：

1. 这个功能不是需要高频打开的日常命令。
2. bare command 更适合承担“入口菜单”角色。
3. 真正的摘要信息与条目列表，应分别交给 `status` 与 `list`。
...
5. Runtime layer
   - `turn_terminal` 走一次性推理决策
   - `schedule_tick` 直接 dispatch 已编译 action

关键原则：

1. 模型负责“理解用户意图并改写成机器可读版本”。
2. 后端负责“只允许受支持的事件、source、action 进入运行时”。
3. `turn_terminal` 与 `schedule_tick` 不共享同一套运行时语义。
4. 运行时不再处理一整篇开放文本，而是处理有限 JSON。
...
1. `turn_terminal`
   - 保留 `sources + decision + draft_actions`
   - 运行时会做一次 one-off 推理，再在候选 action 间合并或仲裁
2. `schedule_tick`
   - 直接编译成 `dispatch_actions`
   - 到时间就直接 dispatch，不再额外做一次 Autopilot 运行时推理
...
### 8.3 `turn_terminal` 路径的 `Autopilot` runtime 应是 one-off
...
### 8.4 `schedule_tick` 不做额外判断，直接 dispatch

`schedule_tick` 与 `turn_terminal` 不同。

这里“到时间了”本身就已经是命中条件，因此 V1 不再多做一轮 Autopilot runtime 推理。

正确模型应是：

1. `/autopilot load` 时把自然语言 schedule 编译成稳定 job entry
2. 同时冻结这个 entry 所需的 `runner contract`
3. scheduler 到点后直接 dispatch 这条 entry 的 `dispatch_actions`
4. 如果 action 是 `new_thread_prompt`，就直接 `launch_turn`

也就是说：

- `schedule_tick -> direct dispatch compiled action`
- 不再经过 `event -> reason -> decide` 这一层

V1 建议对 `schedule_tick` 额外收窄两条约束：

1. 不允许 `same_thread_prompt`
2. 以 `new_thread_prompt` 为主，必要时才允许直接 `feishu_message`

### 8.5 如果 action 拉起 turn，后续复杂行为属于那个 turn
...
### 8.6 因此 action 需要带“能力画像”而不需要带 follow-up
...
1. 定时事件只负责直接拉起一次执行 thread
2. scheduler 侧不再做额外推理
...
10. `turn_terminal` entry 顶层只允许一个 `primary action`；模型若产出多个，编译器必须折叠成一个或直接拒绝
11. `capability_profile` 或额外 MCP grant 必须来自受支持白名单
12. 若 spec 中存在 `schedule_tick` entry，则当前工作区必须能拿到可用的 `runner contract`
13. `schedule_tick` entry 不允许再携带 `decision` block
14. `schedule_tick` entry 不允许 `same_thread_prompt`

这里的原则是：

能在 load 阶段明确失败的，就不要拖到运行时失败。

## 10. `turn_terminal` 的 one-off 决策与多条规则命中处理
...
### 10.7 `schedule_tick` 不走这一套合并器

`schedule_tick` 是另一条更确定性的路径。

V1 建议：

1. 每条 `schedule_tick` entry 在 load 后各自注册成独立 scheduler job
2. 到点后直接 dispatch 该 entry 自己的 `dispatch_actions`
3. 不与别的 schedule entry 做运行时合并仲裁
4. 若两个 schedule entry 恰好同一分钟触发，它们就是两条独立 job，分别执行
...
这里需要把“schedule 类 entry 用谁来启动真正执行 turn”单独收敛成一个运行时合同。
...
1. 给没有“当前 turn 来源实例”的 schedule 事件一个稳定启动执行面
...
但这里的 contract 不是为了给 Autopilot 再跑一轮 reasoner，而是为了直接启动真正的执行 turn。

也就是说：

1. `schedule_tick` 不是看“现在谁在线”
2. 也不是看“最近一次是谁跑过这个工作区”
3. 而是看“这个工作区最后一次成功 load 时绑定下来的 launcher 是谁”
4. 到点后直接用它 dispatch `new_thread_prompt` 或 `feishu_message`
...
4. 定时触发后，不再做额外判断，直接由这个 `runner contract` 拉起执行 turn
...
Autopilot 替换 `/cron` 的配置入口与表达方式，运行基底大体沿用 `scheduler tick -> hidden run -> launch turn` 这条老机制。
...
1. 支持 `Autopilot.md`
2. 支持 bare `/autopilot` 菜单
3. 支持 `/autopilot list`
4. 支持 `/autopilot status`
5. 支持 `/autopilot load`
6. 支持 workspace/rule 两级开关
7. 先把编译结果稳定存起来
...
1. `new_thread_prompt` 的 thread 命名、可见性和默认标题是否需要产品约定
...
4. 把 `turn_terminal` 收敛成 one-off、无工具的决策层
5. 把 `schedule_tick` 收敛成 direct-dispatch 的 job trigger，而不是再做一次运行时判断
6. 把复杂后续行为交给被拉起的 turn 和它被授予的能力去完成
7. 把 `turn_terminal` 和 `schedule_tick` 的执行来源明确分开：前者跟随事件源实例，后者跟随工作区持久化 `runner contract`
8. 用显式 load、workspace/rule 两级开关，以及 `turn_terminal` 的 lane 合并与冲突拒绝，把不确定性关进一个可控边界里
