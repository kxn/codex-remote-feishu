...
func TestDaemonIdleHeadlessCleanupStopsOutsideAppLockAndKeepsStoppingState(t *testing.T) {
	...
	app.managedHeadless["inst-headless-2"] = &managedHeadlessProcess{
		InstanceID:       "inst-headless-2",
		PID:              2468,
		WorkspaceRoot:    "/data/dl/droid",
		DisplayName:      "headless",
	...
	managed := app.managedHeadless["inst-headless-2"]
	if managed == nil {
	...
	if managed := app.managedHeadless["inst-headless-2"]; managed == nil || managed.Status != managedHeadlessStatusStopping {
	...
	if _, ok := app.managedHeadless["inst-headless-2"]; ok {
