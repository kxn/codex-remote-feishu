...
func TestNewThreadTurnStartedPromotesPendingRemoteByCommandWhenInitiatorBlank(t *testing.T) {
	...
	surface := svc.root.Surfaces["surface-1"]
	if surface.RouteMode != state.RouteModePinned || surface.SelectedThreadID != "thread-created" {
		t.Fatalf("expected blank-initiator turn.started to rebind surface, got route=%q selected=%q", surface.RouteMode, surface.SelectedThreadID)
	}
	...
	if !sawPinnedSelection {
		t.Fatalf("expected pinned selection change after blank-initiator turn started, got %#v", events)
	}
}
