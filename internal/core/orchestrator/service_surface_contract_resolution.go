...
func (s *Service) resolveWorkspaceContract(surface *state.SurfaceConsoleRecord, workspaceKey string, backend agentproto.Backend) contractResolution {
	...
	allowSameRoomIndependentContext := s.sameRoomWorkspaceIndependentContextAllowed(surface, workspaceKey)
	resolved := s.resolveHeadlessContract(surface, contractResolutionContext{
		...
		AllowDirectVisible: func(inst *state.InstanceRecord) bool {
			...
			if allowSameRoomIndependentContext && s.instanceClaimedBySameRoomSibling(surface, inst.InstanceID) {
				return false
			}
			return true
		},
