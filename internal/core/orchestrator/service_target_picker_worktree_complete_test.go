...
func TestCompleteTargetPickerWorktreeCreateReplaysPendingTextWhenImmediatelyReady(t *testing.T) {
	now := time.Date(2026, 4, 14, 16, 6, 0, 0, time.UTC)
	svc := newServiceForTest(&now)
	currentWorkspace := createTargetPickerGitRepo(t)
	createdWorkspace := createTargetPickerGitRepo(t)
	...
	svc.attachWorkspace(surface, currentWorkspace)
	svc.storePendingTextInput(surface, "继续这个需求", nil, "msg-pending", "user-1", "msg-pending", nil)
	record := &activeTargetPickerRecord{
		PickerID:             "picker-1",
		OwnerUserID:          "user-1",
		Source:               control.TargetPickerRequestSourceWorktree,
	...
	if svc.hasPendingTextInput(surface) {
		t.Fatalf("expected pending text to be consumed after immediate worktree ready, got %#v", surface.PendingTextInput)
	}
	if surface.ActiveQueueItemID == "" {
		t.Fatalf("expected pending text replay to enqueue first worktree prompt, got %#v events=%#v", surface, events)
	}
	var sawCreateThreadPrompt bool
	for _, event := range events {
		if event.Command != nil && event.Command.Kind == agentproto.CommandPromptSend && event.Command.Target.CreateThreadIfMissing {
			sawCreateThreadPrompt = true
		}
	}
	if !sawCreateThreadPrompt {
		t.Fatalf("expected pending text replay to dispatch create-thread prompt, got %#v", events)
	}
}
