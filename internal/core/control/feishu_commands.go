...
var feishuCommandSpecs = []feishuCommandSpec{
	...
	},
	{
		definition: FeishuCommandDefinition{
			...
			ArgumentFormHint: "gpt-5.5 high",
			ArgumentFormNote: "输入模型名，或输入“模型名 推理强度”。",
			ArgumentSubmit:   "应用",
			Description:      "查看当前模型配置；bare `/model` 会给出当前实例可用模型与手动输入入口。",
			Examples:         []string{"/model gpt-5.5", "/model gpt-5.5 high", "/model clear"},
			ShowInHelp:       true,
			ShowInMenu:       true,
			RecommendedMenu: &FeishuRecommendedMenu{
				Key:         "model",
				Name:        "使用模型",
				Description: "打开模型卡片；如果知道完整 key，也可直接使用 `model_gpt-5.5` 这类直达入口。",
			},
		},
		textPrefixes: []feishuCommandPrefixMatch{
	...
	},
	{
		definition: FeishuCommandDefinition{
			...
			CanonicalMenuKey: "codex_profile",
			ArgumentKind:     FeishuCommandArgumentText,
			ArgumentFormHint: "native",
			ArgumentSubmit:   "切换",
			Description:      "查看当前 Codex Profile；bare `/codexprofile` 会返回可切换的 Profile 下拉卡片。",
			Examples:         []string{"/codexprofile native"},
			ShowInHelp:       true,
			ShowInMenu:       true,
		},
		textPrefixes: []feishuCommandPrefixMatch{
