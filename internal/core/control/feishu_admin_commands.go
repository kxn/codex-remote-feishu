...
func adminCommandSpec() feishuCommandSpec {
	return feishuCommandSpec{
		definition: FeishuCommandDefinition{
			...
			Description:      "打开系统管理入口，并查看管理页、网络模式、自动启动与维护命令。",
			Examples:         []string{"/admin web", "/admin network", "/admin autostart on"},
			ShowInHelp:       true,
			ShowInMenu:       true,
		},
...
func adminSubcommandSpec() feishuCommandSpec {
	return feishuCommandSpec{
		definition: FeishuCommandDefinition{
			...
			ArgumentFormNote: "例如 web、network、network local、autostart on。",
			ArgumentSubmit:   "执行",
