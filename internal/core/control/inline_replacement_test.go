...
func TestAllowsCommandCardResultReplacement(t *testing.T) {
	tests := []struct {
		...
		{
			name: "review follow-up stays append-only to preserve result actions",
			action: Action{
				Kind:    ActionReviewFollowUp,
				Inbound: &ActionInboundMeta{CardDaemonLifecycleID: "life-1"},
			},
			want: false,
		},
...
func TestResolveFeishuFrontstageActionContractLauncherDisposition(t *testing.T) {
	tests := []struct {
		...
		{name: "coworkers enters terminal", kind: ActionCoworkersCommand, want: FeishuFrontstageLauncherEnterTerminal},
