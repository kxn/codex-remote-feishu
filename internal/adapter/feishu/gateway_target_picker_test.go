...
func TestParseCardActionTriggerEventBuildsTargetPickerBackAction(t *testing.T) {
	gateway := NewLiveGateway(LiveGatewayConfig{GatewayID: "app-1"})
	gateway.recordSurfaceMessage("om-card-target-picker-back", "feishu:app-1:user:user-1")
	userID := "user-1"
	event := &larkcallback.CardActionTriggerEvent{
		Event: &larkcallback.CardActionTriggerRequest{
			Operator: &larkcallback.Operator{UserID: &userID},
			Action: &larkcallback.CallBackAction{
				Value: map[string]any{
					"kind":      cardActionKindTargetPickerBack,
					"picker_id": "picker-1",
				},
			},
			Context: &larkcallback.Context{
				OpenChatID:    "oc_1",
				OpenMessageID: "om-card-target-picker-back",
			},
		},
	}
	...
	action, ok := gateway.parseCardActionTriggerEvent(event)
	if !ok {
		t.Fatal("expected target picker back action to parse")
	}
	if action.Kind != control.ActionTargetPickerBack || action.PickerID != "picker-1" {
		t.Fatalf("unexpected target picker back action: %#v", action)
	}
}
